...
DI int bin_count(CountBin const& bin) { return bin.x; }

DI int bin_count(AggregateBin const& bin) { return bin.count; }
                                           ...
                                           IdxT treeid)
{
  ...
  for (IdxT c = 0; c < objective.NumClasses(); ++c) {
    pdf_to_cdf<BinT, IdxT, TPB>(shared_histogram + n_bins * c, n_bins);
    split_len += bin_count(shared_histogram[n_bins * c + n_bins - 1]);
  }
  __syncthreads();

  Split<DataT, IdxT> sp =
    objective.Gain(shared_histogram, shared_quantiles, col, split_len, n_bins);
  __syncthreads();
